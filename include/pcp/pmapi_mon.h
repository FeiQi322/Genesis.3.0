/*
 * Copyright (C) 1995-2000,2009 Silicon Graphics, Inc.
 * ALL RIGHTS RESERVED.
 *
 */
 ...
 */
extern int sgi_pmTimeConnect(int, const char *, sgi_pmTime *); /* mode, port, initState */

/* disconnect from time control master or standalone, free internal state. */
extern int sgi_pmTimeDisconnect(void);

/* return port name (in static buffer), or NULL if standalone */
extern const char *sgi_pmTimeGetPort(void);

/* receive a time control command (may block), returns command (or err < 0) */
extern int sgi_pmTimeRecv(sgi_pmTime *);	/* commandBuffer */

/* send ack that PM_TCTL_STEP has been processed */
extern int sgi_pmTimeSendAck(const struct timeval *);	/* actualFetchTime */

/* send earliest and latest archive bounds */
extern int sgi_pmTimeSendBounds(const struct timeval *, const struct timeval *);

/* send a label and a timezone string */
extern int sgi_pmTimeSendTimezone(const char *, const char *);

/* show/hide the master time control dialog */
extern int sgi_pmTimeShowDialog(int);	/* 0==hide else show */

/* send new position and delta (note: delta is signed) */
extern int sgi_pmTimeSendPosition(const struct timeval *, int);

/* send new VCR mode */
extern int sgi_pmTimeSendMode(int); /* PM_TCTL_VCRMODE_STOP, etc */

/* return a char pixmap representing vcrmode (0..3) dir (-1, 0, 1), live (0, 1), record (0, 1) */
extern const char **sgi_pmTimeGetStatePixmap(int, int, int, int); /* vcrmode, dir, live, record */
