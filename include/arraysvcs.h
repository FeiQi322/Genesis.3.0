/* $Id: arraysvcs.h,v 1.63 2011/12/12 21:46:49 michel Exp $ */
#ifndef __ARRAYSVCS_H
#define __ARRAYSVCS_H
 ...
 *
 * Copyright 1995 - 2008, Silicon Graphics, Inc.
 * ALL RIGHTS RESERVED
 *
...
// Same as AS_NOKEY but indicate a Crypter
#define	AS_KEY_RESERVED_HIGH	0xffffffffffffff
#define	AS_MUNGE_KEY		0xffffffffffffff	/* Authentication MUNGE */
// possibly adding more Crypter later : 
// AS_SSL_KEY			0xfffffffffffffe	/* Authentication SSL ?? */
#define	AS_KEY_RESERVED_LOW	0xfffffffffffff0
#define	AS_KEY_RESERVED_GENERIC	AS_KEY_RESERVED_LOW
...
#ifdef	USE_SSL
#define ASE_SSL		8		/* SSL Protocol error : contains no What Code
					   since we fetch error strings using
					   ssl routine ERR_error_string()
					 */
#endif
...
#define ASSE_MONFORK	36		/* Monitor process fork */
...
/* Error functions */
aserror_t asgeterror();
aserror_t asmakeerror(int, int, int, int);
void asperror(const char *, ...);
const char *asstrerror(aserror_t);
int asperrlog(asserver_t, ash_t ASH, const char *, ...);
int aserrlogstat(asserver_t, ash_t);
...
const char *asgetservername(asserver_t);
...
#define ASCMDIO_ERRLOG		0x04000000	/* Log stderr to a logfile.
						 * ASCMDIO_STDERR, 
						 * ASCMDIO_FULLIO or  
						 * ASCMDIO_OUTERRSHR will
						 * override this flag 
						 */
...
/*
 * Associate a set of hosts with the current ASH to filter broadcasts
 */
int asashhosts(asserver_t, const char *, int, asserver_t *);
...
/* Signal functions - with Ignore PID feature */
int askillash_array_ign(asserver_t, const char *array, ash_t,
				int signum, aspidlist_t *);
int askillash_local_ign(ash_t, int signum, aspidlist_t *);
int askillash_server_ign(asserver_t, ash_t, int signum, aspidlist_t *);
